...
const PageChange = props => {
  ...
  const retriveData = () => {
    ...
    firestore.collection("editshow").onSnapshot(snapshot => {
      ...
      // })
      let editshow = snapshot.docs.map(d => {
        const { id, imgUrl1, placetName, district, province} = d.data()
        console.log(id, imgUrl1, placetName, district, province)
        return { id, imgUrl1, placetName, district, province }
      })
  ...
  const addEditshow = () => {
    ...
    firestore.collection("editshow").doc(id + '').set({ id, imgUrl1, placetName, district, province})
    alert("Add Finish")
  }
                ...
                <MDBCard id='classic-card'>
                  <MDBCardBody className='white-text'>
                    ...
                    <div className='text-center mt-4 ไเระำ-text'>
                      <MDBBtn color="#00bfa5 teal accent-4" onClick={addEditshow}>ADD</MDBBtn>
                    </div>
                  </MDBCardBody>
                </MDBCard>
              </MDBAnimation>
